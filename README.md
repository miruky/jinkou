...
特徴はコーホート追跡で、ピラミッドの階級をクリックするとその出生世代に印が付き、年を動かしても同じ世代を追いかけられる。たとえば1945-50年生まれを選んで再生すれば、その山が70代へ登っていくのが一目で分かる。脇の指標パネルには総人口・三区分割合・老年人口指数・従属人口指数・中位年齢が表示され、年とともに動く。主要年(戦後・バブル期・現在・推計末)へのジャンプ、左右矢印キーでの1年送り、スペースキーでの再生切替にも対応する。比較セレクタで任意の参照年を選ぶと、その年の各階級の幅が破線の輪郭として現在の棒に重なり、年をまたいだ増減を直接見比べられる。
...
const stats = snapshotStats(y2020);
formatPopulation(stats.total); // => 1億2,403万人
stats.medianAge.toFixed(1); // => 中位年齢(歳)
stats.totalDependency.toFixed(2); // => 従属人口指数((年少+老年)/生産年齢)
```

`snapshotStats` は三区分割合に加えて老年人口指数・従属人口指数・中位年齢を返す。中位年齢は5歳階級の累積から線形補間で求める。`PyramidSnapshot` は `{ year, male: number[], female: number[] }`(千人単位、21階級)の素朴な形で、任意の実数年は `interpolateSnapshot(series, 1972.5)` で前後の時点から線形補間できる。
...
- `src/lib/stats.ts` 三区分割合・老年/従属人口指数・中位年齢と日本語表記
